...
//                ExpressionVisitor visitor = ExpressionVisitor.getTablefilterVisitor();
//                visitor.addTablefilter(f.getTable());
//                f.getTable().addTableFilters(visitor.getTablefilters());
//                isEverything(visitor);
    ...
    public boolean isEverything(ExpressionVisitor visitor) {
        switch (visitor.getType()) {
            ...
            //改动
            case ExpressionVisitor.GET_TABLEFILTERS: {
                if (visitor.isUnion) {
                    visitor.addUnionId(visitor.getId());
                    visitor.isUnion = false;
                }
                boolean haveFrom = false;
                haveFrom = isHaveFrom(visitor, haveFrom);
                if (haveFrom) {
                    visitor.setId(visitor.getId() + 2);
                } else {
                    visitor.setId(visitor.getId() + 1);
                }
                break;
            }
            default:
        ...
        visitor.setId(v2.getId());
        return result;

    }

    private boolean isHaveFrom(ExpressionVisitor visitor, boolean haveFrom) {
        for (TableFilter f : filters) {
            Table table = f.getTable();
            visitor.addIds();
            visitor.addTablefilter(f);
            ExparMethod(visitor, f);
            table.addTableFilters(visitor.getTablefilters());
            RowCount(visitor, table);
            getCostRangeIndexa(visitor,table);
            if (table instanceof TableView) {
                haveFrom = true;
                for (Table t:((TableView) table).getTables()){
                    getCostRangeIndexa(visitor,t);
                    RowCount(visitor,t);
                }
                visitor.setId(visitor.getId() + 1);
                visitor.addIds();
                visitor.setId(visitor.getId()-1);
                ExparMethod(visitor, ((Select) ((TableView) table).getViewQuery()).getTopTableFilter());
            }
        }
        return haveFrom;
    }

    private void RowCount(ExpressionVisitor visitor, Table table) {
        visitor.setRowCountApproximation(table.getRowCountApproximation());
        visitor.addRowCountApproximationList();
        visitor.setRowCountApproximationList(visitor.getRowCountApproximationList());
    }

    private void getCostRangeIndexa(ExpressionVisitor visitor,Table table) {
        int totalSelectivity = 0;
        for (int i = 0;i < table.getColumns().length; i++){
            totalSelectivity = 100 - ((100 - totalSelectivity) *
                    (100 - table.getColumn(i).getSelectivity()) / 100);
            //最大行* 百分比 /100
            long distinctRows = (table.getRowCountApproximation() * totalSelectivity  /100);
            if (distinctRows <= 0){
                distinctRows = 1;
            }
        }
            visitor.setTotalSelectvit(totalSelectivity);
            visitor.addTotalSelectvits();
            visitor.setTotalSelectvits(visitor.getTotalSelectvits());
    }

    private void ExparMethod(ExpressionVisitor visitor, TableFilter f) {
        StringBuilder sb=new StringBuilder();
        //判断condition 是否等于null
        if (f.getSelect().getCondition() != null){
            //判断如果包含< > in or
            if ( f.getSelect().getCondition().toString().contains("<") || f.getSelect().getCondition().toString().contains(">") || f.getSelect().getCondition().toString().contains("IN") ||f.getSelect().getCondition().toString().contains("or") ) {
                visitor.setRef("null");
                visitor.addRefs();
                visitor.setRefs(visitor.getRefs());
            }else if (f.getSelect().getCondition().toString().toLowerCase().contains("and")){
                visitor.setRef("const");
                visitor.addRefs();
                visitor.setRefs(visitor.getRefs());
            }else if (!f.getSelect().getCondition().toString().contains("\'\'")){
                   //如果截取完包含数字，那么set一个const
                    if (StringUtils.isNumber(f.getSelect().getCondition().toString().substring(f.getSelect().getCondition().toString().indexOf("="),f.getSelect().getCondition().toString().lastIndexOf(")")).replace("=","").trim())){
                        visitor.setRef("const");
                        visitor.addRefs();
                        visitor.setRefs(visitor.getRefs());
                    }else{
                        //如果截取的没有‘’。
                        visitor.setRef("VDS."+f.getSelect().getCondition().toString().substring(f.getSelect().getCondition().toString().indexOf("="),f.getSelect().getCondition().toString().lastIndexOf(")")).replace("=",""));
                        visitor.addRefs();
                        visitor.setRefs(visitor.getRefs());
                    }
            }
            else if (f.getSelect().getCondition().toString().substring(f.getSelect().getCondition().toString().indexOf("'"),f.getSelect().getCondition().toString().lastIndexOf("'")).replace("\'","").equals("")) {
                 //如果截取完是个""字符串的话 给他一个""
                 visitor.setRef("");
                 visitor.addRefs();
                 visitor.setRefs(visitor.getRefs());
        ...
        if(orderList != null){
            if (sb != null && sb.length()!=0 && sb.toString() != ""){
                sb.append(","+"Using filesort");
            }else
                sb.append("Using filesort");
        }
        if(groupIndex != null){
            if (sb != null && sb.length()!=0 && sb.toString() != ""){
                sb.append(","+"Using temporary Using filesort");
            }else
                sb.append("Using temporary Using filesort");
        }
        if(f.getJoin() != null){
            if (sb != null && sb.length()!=0 && sb.toString() != ""){
                sb.append(","+"Using join buffer");
            }else
                sb.append("Using join buffer");
        }
        if(f.getIndex() != null && f.getIndex().getName()!=null){
            if (sb != null && sb.length()!=0 && sb.toString() != ""){
        ...
        if (sb == null && sb.length()==0 && sb.toString() == ""){
                sb.append("const row not found");
        }
        visitor.setExtra(sb.toString());
        visitor.addExtraList();
        visitor.setExtraList(visitor.getExtraList());
    }
